...
void nexttime(void){
	...
	// Set second to 10 (jump is every 10 seconds)
	second = second +10;
...
void log_data(void){
	...
	save_value8bit(count,0);						// count stored in position 0 in the EEPROM memory array : byte 0 
	save_value8bit(year,1);						 // year stored in position 1 in the EEPROM memory array	: byte 1
	uint8_t temp = (month<<4) | (day>>1);		// month to be stored in the first 4bits (first nibble) and bits 1 through 4 for day to be stored in remainining nibble: byte 2
	save_value8bit(temp,2);						// write into byte 2
	temp = ((1&day) << 7) | (hour);				// only the LSB of "day" byte is stored into the MSB of the 3rd byte of EEPROM array // the remianing 7 bits used to store the hour
	save_value8bit(temp,3);						// write temp into position 3 in the EEPROM memory array : byte 3
	save_value8bit(minute,4);					//minute stored in position 4 in the EEPROM memory array : byte 4
	save_value8bit(second,5);					//second stored in position 5 in the EEPROM memory array : byte 6
...
void display_show_data(uint8_t i, uint16_t temp){
	...
	//print time
	lcd_setcursor(0,2);
	snprintf(str, 16, "%02d:%02d:%02d",hour, minute, second);
	lcd_string(str);
...
void show_data(void){
	...
	char str[16];
	uint8_t count = load_value8bit(0);			// reads count value from byte 0
	year = load_value8bit(1);					// reads year value from byte 1 in EEPROM array
	uint8_t temp1 = load_value8bit(2);			//reads from byte 2 stores in temp1
	month = (temp1 >> 4);						//month is read from the 4 left most bits of byte 2/temp1
	uint8_t temp2 = load_value8bit(3);			// reads from byte 3 and saves as temp2
	day = ((15 & temp1) << 1) | (temp2 >> 7);		// reads right most nibble of temp1 (byte 2) and the MSB of temp2(byte 3), combines the bits and saves as day
	hour = (127 & temp2);						// reads the last 7bits of from temp2(byte 3) and saves as hour
	minute = load_value8bit(4);	
	second = load_value8bit(5);
	uint16_t temp = load_value16bit(6);
	lcd_clear();
	snprintf(str, 16, "Values =%d", count);			// displays count
	lcd_setcursor(0,1);
	lcd_string(str);
	
	lcd_setcursor(0,2);
	snprintf(str, 16, "@%02d.%02d.%02d %02d:%02d:%02d", day, month, year, hour, minute, second);		// displays time
	lcd_string(str);
	//_delay_ms(1000);
