...
void init(void){
	...
	TIMSK1 |= (1 << TOIE1); 	// overflow interrupt enable for TC1
	TIMSK = (1<<TOIE0);
...
ISR(TIMER1_OVF_vect){
	// Your code here
	PORTB |= (1<<PB2);  				//enable the green LED when the Timer/Counter 1 overflows,
}
/**
Compare interrupt
*/
...
ISR(TIMER1_COMPA_vect){ 				//for OCR1A
	PORTB &= ~(1 << PB2);				//disable the green LED when Timer/Counter 1 reaches the value 1000
}
