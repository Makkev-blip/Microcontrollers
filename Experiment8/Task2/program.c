...
void display_log_data(uint8_t count, uint16_t temp){
	...
	lcd_setcursor(0,2);
	lcd_string(str);
	snprintf(str, 8, "%d.%d C",temp/10, temp%10); 				//from hundreds to tens with decimal
	lcd_setcursor(8,2);
	lcd_string(str);
}
/******************************************************************************/
	...
	while(1) {
		...
		if (second == 0){
			if (logged==0){
				...
				save_value8bit(count,0);
				save_value16bit(nowtemp,(count*2)+4);		//saves current temp and sets correct memory address (start from 6bit bcs time takes up other bits
				logged = 1;
			}
		} else {
			logged=0;
...
void uart_output(void){
	...
	if (count==0){ 		//checks if write position is empty
		uart_string("Error! Empty"); // prints error if there is nothing to read
		return;
	}
	...
	uint8_t i;
	for (i = 1; i <= count ; i++){
		temp = load_value16bit(4+2*i);
		snprintf(str,6,"26219");
		uart_string(str);
